...
./melody.sh $INS 120 63 $DUR 68 $DUR 72 $DUR | csvmidi - > melody.midi && timidity -Ow melody.midi > /dev/null
...
sox -D --norm=-0.1 .pre.wav desktop-item.wav reverb 75 fade t 0 2 2
rm -f *.midi .*.wav

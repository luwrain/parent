...
mv drum.wav .drum1.wav
sox --norm=$DRUM_VOL .drum1.wav -r 256000 -c 1 .drum2.wav pad $DRUM_DELAY 5 bass $BASS
sox -D .drum2.wav -c 2 .drum.wav reverb 80 50 100 100 50 5

sox -D -n -r 256000 -c 2 -b 16 .harm1.wav \
    synth 10 sin %1 sin %4 sin %9 \
    fade t 0.4 2 1.5 pad $HARM_DELAY 3 \
    gain $HARM_VOL
sox -D .harm1.wav .harm.wav reverb 100 100 100 100 20 5

./melody.sh $INST 120 90 $DUR | csvmidi - > _note1-1.midi
timidity -Ow _note1-1.midi > /dev/null
sox  --norm=$NOTE1_VOL _note1-1.wav -r 256000 _note1.wav \
    gain -10 \
    pad 0.95 3 \
...
sox  --norm=$NOTE2_VOL _note2-1.wav -r 256000 _note2.wav \
    pad 1.7 3 \
    reverb 100  50 100 100 0 10

sox  -D -n -r 256000 -c 1 -b 16 _noise1.wav \
...
sox  --norm=-25 _high1.wav -r 256000 .high.wav pad 0.5 \

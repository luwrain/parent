...
./melody.sh $INS $VOLUME 73 $DUR 65 $DUR 73 $DUR | csvmidi - > melody.midi && timidity -Ow melody.midi > /dev/null && mv melody.wav .melody-src.wav
...
sox -D .pre.wav area-layout-triple.wav  pad 0 1
rm -f .*.midi .*.wav
